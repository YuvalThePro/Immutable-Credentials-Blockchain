...
public class CredentialService {
    ...
    /**
     * Construct a new CredentialService bound to the given node.
     * 
     * @param node the running (or stopped) Node instance; must not be null
     ...
     * @return a non-null (possibly empty) list of matching Credential objects
     ...
     * local blockchain, meaning it has been finalised in a block.
     * 
     * @param credentialId the credential ID to check; must not be blank
     * @return true if the credential exists on-chain; false otherwise
     * @throws IllegalArgumentException if credentialId is null or blank
     */
     ...
     * major globally-accredited institutions authorised to propose and sign blocks
     * in addition to issuing credentials.
     * 
     * @return true if this node has an active validator identity;
     *         false for plain university nodes and read-only nodes
     */
